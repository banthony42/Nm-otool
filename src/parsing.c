...
/*   Updated: 2017/12/11 22:11:41 by banthony         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm_otool.h"

static int	option_error_handler(t_list **lst, char *cmd, char *options)
{
	int		i;
	char	dash;
	t_data	*d;
	...
	while (l)
	{
		...
		dash = 0;
		d = (t_data*)l->content;
		if (d->token[TYPE] == OPTION)
		{
			while (d->av[++i])
			{
				if (!(ft_strchr(options, (int)d->av[i]))
					|| (dash && d->av[i] == '-') || !ft_strcmp(d->av, "-"))
					return (cmd_info(cmd, d->av, UNKNOWN_OPTION));
...
t_list		*parsing(char **av, char *cmd, char *options)
{
	...
	if (!(option_error_handler(&lst, cmd, options)))
...
static int	get_type(t_data *d)
{
	...
	{
		if (errno == EACCES)
			cmd_info(COMMON, d->av, PERMISSION);
		else if (errno == ENOENT)
			cmd_info(COMMON, d->av, FILE_NOT_FOUND);
		return (1);
	}
	else if ((ret = fstat(d->fd, &d->stat)) < 0)
	{
		cmd_info(COMMON, d->av, FSTAT_ERROR);
		return (1);
	}
	else if (S_ISDIR(d->stat.st_mode))
	{
		cmd_info(COMMON, d->av, IS_A_DIR);
		return (1);
	}
	return (0);
}
...
void		prepare_files(t_list *elm)
{
	...
	if (!elm || !(d = (t_data*)elm->content) || d->token[TYPE] != PATH)
		return ;
	error = 0;
	error = get_type(d);
	...
	{
		cmd_info(COMMON, d->av, MMAP_ERROR);
		error = 1;
	}
...
void		default_file(t_list **lst)
{
	...
	while (l)
	{
		if (((t_data*)l->content)->token[TYPE] == PATH)
			return ;
		l = l->next;
	}
